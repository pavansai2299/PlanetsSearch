...
function App() {
  ...
  const getSearchResults = (srchVal, clrVal, sizeVal, shapeVal) => {
    ...
    fetch(
      `http://localhost:3000/planets?q=${srchVal}&${clrVal}${sizeVal}${shapeVal}`
    )
      .then((res) => res.json())
      .then((res) => {
        // console.log(res);
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([colors, sizes, shapes]) => {
        ...
        if(params.colors || params.shape || params.size){
          setFilters({
            ...filters,
            colors: colors,
            sizes: sizes,
        ...
        setFilters({
          ...filters,
          colors: colors.map(item => ({...item,clicked: false})),
          sizes: sizes.map(item => ({...item,clicked: false})),
          shapes: shapes.map(item => ({...item,clicked: false})),
          updated: true,
        });
      }
      })
  ...
  const handleShareClick = () => {
    ...
    var sq = searchVal;
    var cq = filters.colors
      .filter((item) => item.clicked)
      .map((item) => item.id)
      .join(",");
    var szeq = filters.sizes
      .filter((item) => item.clicked)
      .map((item) => item.id)
      .join(",");
    var shapeq = filters.shapes
      .filter((item) => item.clicked)
      .map((item) => item.id)
      .join(",");
